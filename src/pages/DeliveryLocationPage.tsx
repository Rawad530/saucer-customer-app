...
import { AlertCircle, CheckCircle, MapPin, Loader2, Home, Briefcase, Trash2 } from 'lucide-react';
            ...
            <div className="space-y-2 pb-4 border-b border-gray-700">
              ...
              <div className="flex flex-wrap gap-2">
                {savedAddresses.map((addr) => (
                  <div key={addr.label} className="flex items-center gap-1"> {/* --- WRAPPER DIV --- */}
                    <Button
                      variant="outline"
                      size="sm"
                    ...
                    <Button // --- NEW: Delete button ---
                      variant="ghost"
                      size="icon"
                      // Make the icon slightly smaller and adjust padding for alignment
                      className="text-gray-400 hover:text-red-500 hover:bg-transparent h-8 w-8" 
                      onClick={(e) => {
                        ...
                        if (window.confirm(`Are you sure you want to delete "${addr.label}"?`)) {
                          handleDeleteAddress(addr.label);
                        }
                      }}
                    >
                      <Trash2 className="w-4 h-4" />
                    </Button>
                  </div> // --- MODIFIED: Close wrapper div ---
                ))}
              </div>
            </div>
          )}
